...
> [!NOTE]
> この文書は2026年5月demo世代の固定スナップショットを解説する履歴資料です。
> 2026年8月に公開された現行source、Home Mixer default、モデルprofileとの差分は
...
Home Mixerが重み（現行sourceでは公開defaultあり）で合成し、著者多様性・
ネットワーク内外補正・安全性フィルタを
...
head名metadataがないため、正しい順序を上流へ確認する必要がある。追加でrankerの
`model_params.npz`（圧縮約3.0MB）も調べたが、出力は汎用名
`unembeddings.npy (128, 19)`、履歴action入力は`action_projection.npy (19, 128)`で、
各列のsemantic labelは保存されていなかった。本ツールの
...
python scripts/audit_model_contract.py --ref main --fail-on-drift  # 新規変更時のみ exit 1
```

この監査は2.9GB全体を取得せず、約68KBのRange requestだけでモデル設定を検査する。
既知の不整合を含む契約全体は
[`state/model_contract_baseline.json`](../state/model_contract_baseline.json)へ固定されており、
GitHub ActionsはLFS SHA-256、寸法、README値、action順の新しいdriftを区別して報告する。
